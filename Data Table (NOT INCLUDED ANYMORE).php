...
<head>
    ...
    <!-- Title -->
    <title>Historic Readings | NWQM</title>
    <!-- Favicon -->
		...
		<div class="container">
			<nav class="navbar navbar-expand-lg">
				<div class="header-logo mr-auto">
					<a href="#"><img src="assets/images/logo.png" alt="NWQM"></a>
				</div>
	...
	<div class="main-wrapper">
        ...
        <nav id="sidebar" class="active">
            <ul class="list-unstyled components">
                ...
                <li>
                    <a href="Data Table - Realtime.php" data-toggle="collapse" aria-expanded="false" class="dropdown-toggle list-menu">Data Table </a>
                </li>
                <li>
                    <a href="Historic Readings.php" data-toggle="collapse" aria-expanded="false" class="dropdown-toggle list-menu">Historic Readings </a>
                </li>
                <li>
                    <a href="Data Charts.php" class="list-menu">Data Charts - Comparative Analysis</a>
                </li>
                <li>
                    <a href="Data Charts - Realtime.php" class="list-menu">Realtime Readings</a>
                </li>
                </li>
                <li>
		    		...
		    		<div class="row">
		    			<div class="content col-12">
		    				...
		    				<ul>
		    					...
		    					<li class="current"><a href="#">Historic Readings</a></li>
		    				</ul>
		    			</div>
		    		</div>
		    	</div>
...
<center>
  ...
  <tr>
    ...
    function getJSON(){
        ...
        $json_text1 = file_get_contents("assets/ajax/newDatOutput.json");

        $data_json=json_decode($json_text,TRUE); //converting json string object to json object
        $data_json1=json_decode($json_text1,TRUE);
        $resultString='{"aaData":';
        print_r($data_json1["rows"]);
        ...
        for($x=0;$x<count($data_json1["rows"]);$x++){
          $resultString.='['.($data_json1["rows"][$x]["timestamp"]).','.($data_json1["rows"][$x]["turbidity"]).','.($data_json1["rows"][$x]["pH"]).','.($data_json1["rows"][$x]["dissolvedOxygen"]).','.($data_json1["rows"][$x]["conductivity"]).','.($data_json1["rows"][$x]["temperature"]).']';
           if ($x<(count($data_json1["rows"])-1)){
              $resultString.=',';
            }
        }
                    ...
                    <div class="panel-body" style="width: 100%">
                        ...
                        <thead class="thead-dark">
									...
									<tbody>
                                        ...
                                        for($x=count($data_json1["rows"])-1;$x>=0;$x--){
                                            echo '<tr>';
                                            $tempDiff=abs($prevTemp-floatval($data_json1['rows'][$x]['temperature']));
                                            $prevTemp=floatval($data_json1['rows'][$x]['temperature']);
                                            $wqi=$tempDiff;
                                            echo '<td>'.$data_json1['rows'][$x]['timestamp'].'</td>'.'<td>'.$data_json1['rows'][$x]['turbidity'].'</td>'.'<td>'.$data_json1['rows'][$x]['pH'].'</td>'.'<td>'.$data_json1['rows'][$x]['dissolvedOxygen'].'</td>'.'<td>'.$data_json1['rows'][$x]['conductivity'].'</td>'.'<td>'.$data_json1['rows'][$x]['temperature'].'</td>'.'<td>'
                                                .$wqi.'</td>';
                                            echo '</tr>';
                                        }
                                        ?>
